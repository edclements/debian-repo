...
packages/ruby-install_0.7.0-1_amd64.deb: packages/ruby-install_0.7.0-1/debian/changelog packages/ruby-install_0.7.0.orig.tar.gz
	cd packages/ruby-install_0.7.0-1; debuild -i -us -uc -b
...
packages/stterm_0.8.2-1.1_amd64.deb: packages/stterm/debian/changelog packages/stterm_0.8.2.orig.tar.gz
	cd packages/stterm; debuild -i -us -uc -b

packages/hedaleth-base_1.0_all.deb:
	cd packages; equivs-build hedaleth-base

sync: $(PACKAGES)
	aws s3 sync --region eu-west-2 --acl public-read --exclude "*" --include "pool/*" --include "dists/*" . s3://debian.hedaleth.net/
